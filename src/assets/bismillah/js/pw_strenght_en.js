...
function passwordStrengthCheck(password1, password2, passwordsInfo)
{
	...
	$(password1).on('keyup', function(e) {
		...
		{
			passwordsInfo.removeClass().addClass('goodpass').html("Good password.");
		}
		else if(WeakPass.test(password1.val()))
    	{
			passwordsInfo.removeClass().addClass('stillweakpass').html("Your password is too dangerous.");
    	}
		else
		{
			passwordsInfo.removeClass().addClass('weakpass').html("Password (English, numbers, at least 5 special characters)");
		}
	});
